...
#ifndef __CAUSAL_TREE_LOSSES_H__
#define __CAUSAL_TREE_LOSSES_H__
#include <cmath>
#include <algorithm>
#include <iostream>
#include <limits>
...
_causal_tree_loss_on_feature(const py::array_t<dtype> &psudo_outcome, const py::array_t<idtype> &counts,
                             const std::vector<size_t> &bin_range, size_t leaf_idx, size_t feature_idx, size_t treated_ts,
                             double coeff, dtype variance, double min_var_rate, double imbalance_penalty,
                             int min_point_num_node) {
    size_t n_bins = psudo_outcome.shape(2);
    size_t n_treatments = psudo_outcome.shape(3);
    size_t n_outs = psudo_outcome.shape(4);
    ...
    std::vector left_respond(n_outs, dtype(0)), right_respond(n_outs, dtype(0));
    ...
    for (size_t b = 0; b < n_bins; b++) {
        for (size_t d = 0; d < n_treatments; d++) {
            ...
            for (size_t t = 0; t < n_outs; t++) {
                right_respond[t] += psudo_outcome.at(leaf_idx, feature_idx, b, d, t);
            }
        }
    }
    for (size_t j = 0; j < bin_range[1]; ++j) {
        ...
        for (size_t d = 0; d < n_treatments; ++d) {
            ...
            for (size_t t = 0; t < n_outs; ++t) {
                left_respond.at(t) += psudo_outcome.at(leaf_idx, feature_idx, j, d, t);
                right_respond.at(t) -= psudo_outcome.at(leaf_idx, feature_idx, j, d, t);
            }
        }
        ...
        if (imbalance_penalty > 0) {
            _imbalance_loss = imbalance_penalty * (1.0 / left_tot_cnt + 1.0 / right_tot_cnt);
        }
        // calculate loss: (\sum_{i\in L} \pho_i)^2 + (\sum_{i\in R} \pho_i)^2
        dtype _pre_loss = 0, _post_loss = 0;
        for (size_t t = 0; t < n_outs; t++) {
            auto sum_sq =
                std::pow(left_respond.at(t), 2) / left_tot_cnt + std::pow(right_respond.at(t), 2) / right_tot_cnt;
            if (t < treated_ts) {
                _pre_loss += sum_sq / treated_ts;
            } else {
                _post_loss += sum_sq / (n_outs - treated_ts);
            }
        }
...
template <typename dtype, typename idtype>
std::unordered_map<size_t, std::tuple<size_t, size_t, dtype>> causal_tree_splitting_loss(
    std::unordered_map<size_t, std::unordered_map<size_t, std::vector<size_t>>> candidates,// {leaves: {features: bins}}
    const py::array_t<dtype> &psudo_outcome, // shape: n_leaves, n_feature, n_bins, n_treatment, n_outs
    // const py::array_t<dtype>& weight, // shape: n_leaves, n_feature, n_bins, n_treatment
    const py::array_t<idtype> &counts, // shape: n_leaves, n_feature, n_bins, n_treatment
    const json11::Json &config) {
    size_t n_leaves = psudo_outcome.shape(0);
    size_t n_features = psudo_outcome.shape(1);
    size_t n_bins = psudo_outcome.shape(2);
    size_t n_treatments = psudo_outcome.shape(3);
    size_t n_outs = psudo_outcome.shape(4);
    ...
    for (auto && items : candidates) {
        ...
        std::vector<dtype> total_count(n_treatments, 0);
        for (size_t i = 0; i < n_bins; ++i){//shape: n_leaves, n_feature, n_bins, n_treatment
            for (size_t d = 0; d < n_treatments; ++d){
                total_count[d] += counts.at(n, 0, i, d);
            }
        }
        dtype variance = total_count.at(1) * total_count.at(0)
                         / std::pow(std::accumulate(total_count.begin(), total_count.end(), dtype(0)), 2);
        ...
        for (auto &&item : features) {
            ...
            if (threads > 1) {
                future_losses[n].push_back(pool.enqueue(_causal_tree_loss_on_feature<dtype, idtype>, psudo_outcome,
                                                        counts, item.second, n, feature_idx, t0, coeff, variance, min_var_rate,
                                                        imbalance_penalty, min_point_num_node));
            } else {
                auto temp = _causal_tree_loss_on_feature<dtype, idtype>(psudo_outcome, counts, item.second, n,
                                                                 feature_idx, t0, coeff, variance, min_var_rate,
                                                                 imbalance_penalty, min_point_num_node);
                losses[n].push_back(std::move(temp));
                // INFO("leaf:%d  feature:%d", n, feature_idx);
            }
        }
    }
    // get future results
    if (threads > 1){
        for (auto &leaf_item : future_losses) {
            ...
            for (auto &feature_item : leaf_item.second) {
                temp.push_back(std::move(feature_item.get()));
            }
    ...
    for (auto &leaf_item : losses) {
        // INFO("size of losss:%d", leaf_item.second.size());
...
#endif // __CAUSAL_TREE_LOSSES_H__