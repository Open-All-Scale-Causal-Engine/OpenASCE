...
class ParamFactory(object):
    def __init__(self):
        ...
        self.params = {
            "model_name": "dnn_base",
            # default
            "hidden_units": [512, 16, 1],
            "hidden_units_emb": [1024, 64],
...
def get_dataset():
    ...
    feature_names = column_names[0:10]
    confounder_names = column_names[10:15]

    logger.info("Features: {}".format(feature_names))
    logger.info("Confounder: {}".format(confounder_names))
    ...
    def pack_features_vector(features, labels):
        feature = tf.stack(list([features[i] for i in feature_names[:-1]]), axis=1)
        confounder = tf.stack(
            list([features[i] for i in confounder_names[:-1]]), axis=1
        )
        labels = tf.reshape(labels, [batch_size, 1])
        return (
            tf.cast(feature, tf.float32),
            labels,
            {"feature": feature, "confounder": confounder},
        )
