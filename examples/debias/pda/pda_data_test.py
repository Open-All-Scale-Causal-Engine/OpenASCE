...
from openasce.extension.debias import PDADebiasModel
...
def get_dataset():
    ...
    def pack_features_vector(features, labels):
        mit = tf.cast(features["W"], tf.int32)
        features = tf.stack(list([features[i] for i in feature_names[:-1]]), axis=1)
        labels = tf.reshape(labels, [batch_size, 1])
        mit = tf.reshape(mit, [batch_size, 1])
        return tf.cast(features, tf.float32), labels, {"feature": features, "mit": mit}
