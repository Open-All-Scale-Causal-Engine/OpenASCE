...
from openasce.inference.t_model import TModel
from openasce.utils.logger import logger


def get_regression_dataset():
    train_dataset_url = "https://github.com/CamDavidsonPilon/lifelines/raw/master/lifelines/datasets/regression.csv"
    # column order in CSV file
    column_names = ["var1", "var2", "var3", "T", "E"]
    ...
    def pack_features_vector(features, labels):
        treatmeat = tf.cast(features["T"] >= 10, tf.int32)
        features = tf.stack(list([features[i] for i in feature_names[:-1]]), axis=1)
        return tf.cast(features, tf.float32), labels, treatmeat
...
def test_model():
    ...
    train_dataset, test_dataset = get_regression_dataset()
    model = TModel(
        hidden_units={"base": [16], "test": [16, 1], "control": [16, 1]}, lr=0.1
    )
    model.fit(Z=train_dataset, num_epochs=200)
    model.estimate(Z=test_dataset)
